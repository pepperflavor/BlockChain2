...
/*
    ...
    트랜잭션 객체가 생겼을 때 UTXO배열 안에서 unspentTxOut 객체 들이 있는 배열 안에서 unspentTxOUt 객체를 제거하고 트랜잭션을 총해
    새로생긴 unspentTxOut 객체들을 UTXO에 추가 시켜주는 작업을 하고 
    UTXO 내용을 업데이트 해준다.

    트랜잭션 풀이라는 공간을 만들고 트랜잭션 객체가 생성 됐을때 만들어진 ㅌ츠랜잭션을 
    트랜잭션 풀에 담아준다. 그리고 블록체인 네트워크 상에 연결된 노드들은 트랜잭션 풀의 내용을 공유한다.
    블록 마이닝 할 때 트랜잭션 풀 안에 있는 트랜잭션 객체를 사용해 샤ㅐ로 생성되는 블록의 data 속성 값으로 넣어준다.
    그래서 필요한게 블록이 마이닝 될 때 트랜잭션 풀을 업데이트 해주는 부분이 필요하다.
    ...
    예전엔 윈도우에서 Linux를 쓰려면 가상머신을 설치해서 써야했는데

    컴퓨터에 메모리나 CPU 등등 자원을 쪼개서 OS를 따로 설치해서 사용할 수 있는데
    이러면 컴퓨터 성능이 줄어들어 비효율적인 단점이 있다.
    WSL은 이 단점을 보완하고 개발 환경을 쉽게 구축가능

    WSL2을 설치해서 사용할 예정이다
    윈도우에서 Linux 명령어를 사용할 수 있게 도와주는 툴 (WSL2)

    제어판 -> 프로그램 -> 프로그램 및 기능 -> window 기능 끄기.켜기
    "Linux용 window 하위 시스템" or "windows Subsystem for Linux" 이걸 체크

    윈도우 키 + S 검색창에 설정 검색 -> 정보 -> 윈도우 버전확인 
    만약 버전이 낮다면 업데이트를 해야한다 그리고
      관리자 모드에서 성능 탭 -> CPㅕ 부분에서 가상화 가 사용중이 아니라면 다음 링크를 참고해서 설정을 해줘야함
    <
