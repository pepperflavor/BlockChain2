...
// curl -X POST -H "content-type : application/json" --data "{\"jsonrpc\" : \"2.0\",\"method\" : \"eth_accounts\"}" http://localhost:8545
 ...
 =======================================================
    ...
    변경하고 싶으면 npm ganashe-cli --chainId "정하고 싶은 체인아이디"


    CMD에서 WSL으로 접속한다음 가나쉬를 켜야한다
     명령어 : npx ganache-cli
    ...
    프론트와 메타마스크 연결
    Dapp을 만드는 기초 작업중 하나
    트랜잭션을 보낼 때 사용자의 개인키를 사용하면 서명을 만드는 과정을 거쳐
    개인키를 서버에 저장하는 것은 많이 위험하기 때문에 메타마스크와 같은 지갑 프로그램을 사용한다

    사용자의 개인키는 메타마스크가 주체가 되고 프론트에서 메타 마스크에 요청을 주고
    요청을 메타마스크가 받아서 사용자의 개인키를 이용해 서명을 만들어 트랜잭션을 발생시킨 후 
    블록체이 네트워크로 요청을 보낸다
    메타마스크는 프론트에서 보낸 요청으로 트랜잭션을 발생시켜 블록체인 네트워크로 보낸다는 것
    ...
    쇼핑몰을 예를 들어 상품들이 등록되어있고 사용자가 코인이나 토큰으로 구매를 할 때
    상품데이터 까지 블록체인 네트워크에 저장하면 너무 비효율적이기 때문에
    프론트, 백엔드 같이 구현해서 상품데이터는 데이터 베이스에 저장하고 상품 목록을 불러오고 
    특정 상품의 특정 상품의 정보들을 가져올땐 백엔드 서버에 요청해서 가져오는 방식


    프론트에서 메타 마스크로 바로 요청을 보내서 트랜잭션을 발생시키는 방법과
    백엔드에서 서명을 제외한 트랜잭션 객체를 만들어서 프론트에 전달하고 프론트에서
    백엔드로부터 받은 트랜잭션 객체를 메타마스크로 전달해 서명과함께 트랜잭션을 보내는 방법
    백엔드에서 사용자의 개인키를 가지고 서명을 만들어서 서명까지 포함된 트랜잭션을 블록체인 네트워크로 바로 보내는 방법


    중요한건 사용자의 개인키로 서명을 만드는게 우리가 아니고 서명을 만들어주는 것은 메타마스크가 된다는 것
